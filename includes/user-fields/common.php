 ...
 */
function codobookingsuf_get_option_name() {
 ...
 */
function codobookingsuf_user_field_default_item() {
    return [
        'label'    => '',
        'name'     => '',
        'type'     => 'text',
 ...
 */
function codobookingsuf_get_default_fields() {
 ...
 */
function codobookingsuf_sanitize_fields_array( $raw ) {
    if ( is_array( $raw ) ) {
        $data = $raw;
    } else {
        $data = json_decode( wp_unslash( $raw ), true );
        if ( ! is_array( $data ) ) {
            return json_encode( codobookingsuf_get_default_fields() );
        }
    }
    ...
    foreach ( $data as $item ) {
        ...
        $field = codobookingsuf_user_field_default_item();
        ...
        $counter = 1;
        while ( codobookingsuf_name_exists_in_array( $field['name'], $clean ) ) {
 ...
 */
function codobookingsuf_name_exists_in_array( $name, $arr ) {
    foreach ( $arr as $item ) {
 ...
 */
function codobookingsuf_get_global_fields() {
    $raw = get_option( codobookingsuf_get_option_name(), wp_json_encode( codobookingsuf_get_default_fields() ) );
 ...
 */
function codobookingsuf_get_fields_for_calendar( $post_id ) {
    $meta = get_post_meta( $post_id, '_codobookings_user_fields', true );
    if ( ! is_array( $meta ) ) return codobookingsuf_get_global_fields();
    ...
    return codobookingsuf_get_global_fields();
}
 ...
 */
function codobookingsuf_render_field_html( $field, $value = '' ) {
    $default = codobookingsuf_user_field_default_item();
    ...
    $name_attr = esc_attr( 'codobookingsuf_' . $field['name'] );
    $label     = esc_html( $field['label'] );
    $hint      = esc_attr( $field['hint'] );
    $required  = $field['required'] ? ' required' : '';
    $html = '<div class="codobookingsuf-field codobookingsuf-field-' . esc_attr( $field['type'] ) . '">';
    $html .= '<label for="' . $name_attr . '">' . $label;
    if ( $field['required'] ) $html .= ' <span class="codobookingsuf-required-star" aria-hidden="true">*</span>';
    $html .= '</label>';
 ...
 */
function codobookingsuf_render_user_fields_for_calendar( $calendar_id ) {
    $fields = codobookingsuf_get_fields_for_calendar( $calendar_id );
    if ( empty( $fields ) ) return '';

    ob_start();
    echo '<div class="codobookingsuf-user-fields-wrapper">';
    foreach ( $fields as $field ) {
        echo codobookingsuf_render_field_html( $field );
    }
    echo '</div>';
...
add_action( 'codobookings_before_calendar', function( $calendar_id ) {
    ...
    if ( $position === 'before' ) {
        echo codobookingsuf_render_user_fields_for_calendar( $calendar_id );
    }
});
 ...
 */
add_filter( 'codobookings_before_booking_insert', 'codobookingsuf_capture_user_fields', 10, 1 );
function codobookingsuf_capture_user_fields( $booking_data ) {
    ...
    foreach ( $fields as $field ) {
        $key = 'codobookingsuf_' . $field['name'];
        $raw = null;
 ...
 */
add_action( 'codobookings_after_ajax_create_booking', 'codobookingsuf_store_user_fields_meta', 10, 2 );
function codobookingsuf_store_user_fields_meta( $booking_id, $booking_data ) {

    $saved = isset( $booking_data['user_fields_data'] ) ? $booking_data['user_fields_data'] : [];

    if ( ! empty( $saved ) ) {
        update_post_meta( $booking_id, '_codobookingsuf_user_fields', wp_json_encode( $saved ) );
    }
}

add_action( 'add_meta_boxes_codo_booking', 'codobookingsuf_add_booking_user_fields_metabox' );
function codobookingsuf_add_booking_user_fields_metabox() {
    add_meta_box(
        'codobookingsuf-booking-fields',
        __('User Fields Data', 'codobookings'),
        'codobookingsuf_render_booking_user_fields_metabox',
        'codo_booking',
        'normal',
        'default'
    );
...
function codobookingsuf_render_booking_user_fields_metabox( $post ) {

    $json = get_post_meta( $post->ID, '_codobookingsuf_user_fields', true );
