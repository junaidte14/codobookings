(function($) {
    ...
    function collectUserFieldsData(calendarId) {
        ...
        const fieldsWrapper = container ? container.querySelector('.codobookingsuf-user-fields-wrapper') : null;
        ...
        inputs.forEach(input => {
            ...
            if (!name || !name.startsWith('codobookingsuf_')) return;
    ...
    function validateUserFields(calendarId) {
        ...
        for (let input of requiredInputs) {
            if (input.type === 'checkbox') {
                if (!input.checked) {
                    const label = input.closest('.codobookingsuf-field')?.querySelector('label')?.textContent || 'A required field';
